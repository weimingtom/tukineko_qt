        ...
        if (!localFile.exists()) {
            ...
            while ((this->line = readLine()) != QString()) {
                ...
                for (; i < this->line.length(); i++) {
                    if (QString(" \t").indexOf(this->line[i]) == -1) {
                        break;
                    }
                }
                if (i > 0) {
...
void NScripter::loadGlobalData()
{
    try {
        ...
        if (localFile.exists() == true) {
            ...
            this->tn->loadValueStorage(new java::io::InputStream(&localByteArrayInputStream), this->nd->valueNum, this->nd->valueStr, 200, 4095);
...
void NScripter::saveGlobalData()
{
    try {
        ...
        this->tn->saveValueStorage(new java::io::OutputStream(&localByteArrayOutputStream),
    ...
    try {
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < arrayOfByte.length(); i++) {
                arrayOfByte[i] = arrayOfByte[i] ^ 0x84;
            }
...
void NScripter::loadLocalData(const QString& paramString)
{
    ...
    try {
        ...
        tn->loadValueStorage(new java::io::InputStream(&localByteArrayInputStream), this->nd->valueNum,
...
void NScripter::saveLocalData(const QString& paramString)
{
    ...
    try {
        ...
        tn->saveValueStorage(new java::io::OutputStream(&localByteArrayOutputStream), this->nd->valueNum, this->nd->valueStr, 0, 199);
