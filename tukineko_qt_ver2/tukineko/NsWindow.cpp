...
void NsWindow::paintB()
{
    ...
    if (this->ndata->bgColor.getRGB() != 0) {
        java::awt::Color_ color(getColor(&this->ndata->bgColor));
        this->frmBuffBG->setColor(&color);
        ...
        localImage = new java::awt::Image_(NsImageCache::get(this->ndata->bgImage));
        if (localImage->m_bufferBmp != nullptr) {
    ...
    if ((this->ndata->btnVisible == true) && (this->ndata->btnSel != -1)) {
        ...
        localImage = new java::awt::Image_(NsImageCache::get(this->ndata->btnImage));
        if (localImage->m_bufferBmp != nullptr) {
    ...
    for (; j < 3; j++) {
        if (!this->ndata->shell[j].image.isEmpty()) {
            ...
            localImage = new java::awt::Image_(NsImageCache::get(this->ndata->shell[j].image));
            if (localImage->m_bufferBmp != nullptr) {
    ...
    for (j = 0; j < 50; j++) {
        if ((this->ndata->sprite[j].visible != true)
                || ((localImage = new java::awt::Image_(NsImageCache::get(this->ndata->sprite[j].image)))->m_bufferBmp == nullptr)) {
            continue;
        }
...
void NsWindow::paintF()
{
    if (this->ns->storageState != 0) {
        ...
        {
            java::awt::Image_ *localImage;
            localImage = new java::awt::Image_(NsImageCache::get(this->ns->path + "MOON.PNG"));
            if (localImage->m_bufferBmp != nullptr) {
                this->frmBuffFG->drawImage(localImage, 5, 70, this);
            } else {
                this->frmBuffFG->drawRect(5, 70, 100, 100);
            }
            localImage = new java::awt::Image_(NsImageCache::get(this->ns->path + "PLUS.PNG"));
            if (localImage->m_bufferBmp != nullptr) {
                this->frmBuffFG->drawImage(localImage, 110, 70, this);
            } else {
                this->frmBuffFG->drawRect(110, 70, 100, 100);
            }
            localImage = new java::awt::Image_(NsImageCache::get(this->ns->path + "KAGETU.PNG"));
            if (localImage->m_bufferBmp != nullptr) {
        ...
        if ((this->ndata->text != nullptr) && (this->ndata->textVisible == true)) {
            ...
            for (int k = 0; k < this->ndata->text->getY(); k++) {
                ...
                if (this->ndata->text->getAttr(k) == true) {
                    NsColor color(this->ndata->text->getColor(k));
                    java::awt::Color_ color_(getColor(&color));
                    this->frmBuffFG->setColor(&color_);
                } else {
    ...
    this->frmBuffFG->setColor(&java::awt::Color_::black);
    QString str1 = QString::number(java::lang::Runtime::getRuntime()->freeMemory());
    QString str2 = QString::number(java::lang::Runtime::getRuntime()->totalMemory());
...
void NsWindow::blt(int j, int k, int m, int n, int i1, int i2)
{
    ...
    java::awt::Image_ *localImage = new java::awt::Image_(NsImageCache::get(this->ndata->btnImage));
    if (localImage->m_bufferBmp != nullptr) {
...
{
    NsResource::initSar(filename);//FIXME: this);
}

void NsWindow::initNsa(const QString &filename)
{
    NsResource::initNsa(filename);//FIXME: this);
}
...
{
    QDataStream ds(paramInputStream->device());
    NsValueStorage::load(ds, paramArrayOfInt,
...
{
    QDataStream ds(paramOutputStream->device());
    NsValueStorage::save(ds, paramArrayOfInt,
            paramArrayOfString, paramInt1, paramInt2);
}

int NsWindow::getImageWidth(const QString &image)
{
    java::awt::Image_ *localImage = new java::awt::Image_(NsImageCache::get(image));
    if (localImage->m_bufferBmp != nullptr) {
